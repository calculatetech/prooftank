# ProofTank Lite design

## Purpose

ProofTank Lite is the smallest ProofTank process for a low-risk Codex change.
...
The future `prooftank-lite-contract` preset must add only:
...
- preset `prooftank-lite-contract`.
- workflow `prooftank-lite`.
- bundle `prooftank-lite`.
...
ProofTank Standard. The future release can reuse the approved Ponytail bytes.
...
- no ProofTank runtime, installer, daemon, database, or adapter is added.
