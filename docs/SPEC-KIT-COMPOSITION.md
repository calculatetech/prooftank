...
ProofTank uses `append` for `spec-template`, `plan-template`, and
`tasks-template`. Spec Kit keeps its core templates as the base. The ProofTank
contract adds only the missing scope and evidence sections.

Install a local preset with:

```bash
specify preset add --dev ./presets/prooftank-contract --priority 10
```
...
ProofTank reuses the core Spec Kit commands. It starts with a deterministic
...
```bash
specify workflow add --dev ./workflows/prooftank-standard
```
...
directories or Codex skills. A local ProofTank install must install the preset,
...
`prooftank_source` to the ProofTank checkout:

```bash
specify bundle validate --path "$prooftank_source/bundles/prooftank-standard" --offline
specify bundle install "$prooftank_source/bundles/prooftank-standard" --offline
```

Offline validation cannot prove that an unresolved remote reference exists. It
reports a warning. The ProofTank validation installs both local components
...
ProofTank Standard has no active extension. A future extension must have an
...
is source metadata, not ProofTank evidence. Default catalog placement also does
not replace ProofTank qualification.

The built-in catalog URLs point at the Spec Kit `main` branch. Their contents
can therefore change while the ProofTank Spec Kit commit stays pinned. A
ProofTank component record must keep its own exact release or commit, archive
...
source warning. ProofTank must review either path before use.

ProofTank does not publish a hosted catalog. A public catalog needs hosted,
versioned downloads. PM-036 publishes the inspectable replacement release
path `releases/prooftank-standard/0.2.0/`. It uses native local-directory
...
`.agents/skills/`. ProofTank adds no installer or package manager.
