# Source map

These sources shaped the bootstrap direction. Verify current versions before implementation.

## Core substrate and harness engineering

- OpenAI Harness Engineering: https://openai.com/index/harness-engineering/
- GitHub Spec Kit: https://github.com/github/spec-kit
- Pi: https://pi.dev/ and https://github.com/earendil-works/pi
- OpenHarness: https://github.com/HKUDS/OpenHarness
- Best of Agent Harnesses: https://github.com/RyanAlberts/best-of-Agent-Harnesses

## Specialist providers

- Ponytail: https://github.com/DietrichGebert/ponytail
- SimpleEnglish: https://github.com/AminBlg/SimpleEnglish
- Codebase Memory MCP: https://github.com/DeusData/codebase-memory-mcp
- CodeGraph: https://github.com/colbymchenry/codegraph
- NeuroArxiv: https://github.com/UditAkhourii/neuroarxiv
- ADHD: https://github.com/UditAkhourii/adhd
- Cavekit: https://github.com/JuliusBrussee/cavekit
- Cavemem: https://github.com/JuliusBrussee/cavemem
- Caveman: https://github.com/JuliusBrussee/caveman

## Research corpus

- arXiv computer science archive: https://arxiv.org/archive/cs

## Use of these sources

- Upstream documentation supplies candidate capability and integration facts.
- ProofTank qualification must verify source code and runtime behavior.
- arXiv content supplies prior art, not automatic authority.
- Community catalogs supply discovery, not approval.
