...
ProofTank converts a software idea into a bounded implementation process and an evidence-backed delivery.

ProofTank uses existing coding agents and specialist tools. It does not replace them.
...
- ProofTank selects, qualifies, pins, configures, and tests the composition.
...
A user must not migrate to a ProofTank-specific agent runtime.
...
ProofTank gives the implementation agent a reviewed work packet that defines both:
...
- A `prooftank-lite` design and one working `prooftank-standard` composition.
- A postponed `prooftank-critical` roadmap item, not a workflow.
- One working `prooftank-standard` bundle.
- Codex conformance.
...
historical Proofmill Standard arm, ProofTank's predecessor. Version 0 therefore
claims a pinned, bounded Codex process, not lower lifecycle cost or higher
