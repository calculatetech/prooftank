# Proofmill Standard bundle

For a versioned, checksummed Codex install, use
`releases/proofmill-standard/0.1.0/README.md`. The source-checkout procedure
...
init --here` in the Proofmill source repository.

Set `proofmill_source` to this checkout and `target_project` to an empty target
directory. Then run:

```bash
proofmill_source=/absolute/path/to/proofmill
...
"$speckit_venv/bin/specify" preset add --dev \
  "$proofmill_source/presets/proofmill-contract" --priority 10
"$speckit_venv/bin/specify" workflow add --dev \
  "$proofmill_source/workflows/proofmill-standard"
"$speckit_venv/bin/specify" bundle validate \
  --path "$proofmill_source/bundles/proofmill-standard" --offline
"$speckit_venv/bin/specify" bundle install \
  "$proofmill_source/bundles/proofmill-standard" --offline
```
...
and the Codex-normalized SimpleEnglish `1.2.0` from this Proofmill release into
`.agents/skills/`:

```bash
provider_checkout=$(mktemp -d /tmp/proofmill-providers.XXXXXX)
git clone \
...
cp -R \
  "$proofmill_source/releases/proofmill-standard/0.1.0/skills/simple-english" \
...
proofmill-standard` and `bundle list`.

To disable the contract, run `specify preset disable proofmill-contract`. To
enable it again, run `specify preset enable proofmill-contract`.

To remove the composition, run:

```bash
specify bundle remove proofmill-standard
specify workflow remove proofmill-standard
specify preset remove proofmill-contract
