# ProofTank Standard 0.2.0 for Codex

This directory is the versioned local release. It contains the ProofTank
...
Set `prooftank_release` to this directory and `target_project` to an empty
target directory. Then run:

```bash
set -e
prooftank_release=/absolute/path/to/releases/prooftank-standard/0.2.0
...
(cd "$prooftank_release" && sha256sum -c SHA256SUMS)
...
"$speckit_venv/bin/specify" preset add --dev \
  "$prooftank_release/components/prooftank-contract" --priority 10
"$speckit_venv/bin/specify" workflow add --dev \
  "$prooftank_release/components/prooftank-standard-workflow"
mkdir -p .agents/skills
test ! -e .agents/skills/ponytail
test ! -e .agents/skills/simple-english
cp -R "$prooftank_release/skills/ponytail" .agents/skills/ponytail
cp -R "$prooftank_release/skills/simple-english" \
...
"$speckit_venv/bin/specify" workflow info prooftank-standard
```
